...
BENEFIT_DETAILS_DATA = {
    ...
    # 'plan' is disabled in the HTML, so we skip it
...
def fill_benefit_details(page: Page):
...
# --- Modified function (now async because get_page_css is async) ---
def generate_tob_preview_pdf(page: Page, context: BrowserContext, output_dir: str = ".", filename_prefix: str = "tob_preview"):
    ...
    Returns:
        Tuple (pdf_path, html_path) or (None, html_path) or (None, None).
    """
    ...
    try:
        ...
        # html_filename = f"{safe_prefix}_with_css_{timestamp}.html" # Indicate CSS included
        ...
        pdf_page.pdf(
            ...
            margin={'top': '20px', 'bottom': '20px', 'left': '20px', 'right': '20px'},
             # Adjust scale if needed after checking output
             # scale=0.9
        )
        ...
        with sync_playwright() as p:
            ...
            if login(page):
                ...
                if navigate_to_new_quote(page):
                    page.wait_for_timeout(2000)
                    if fill_quote_form(page):
                        page.wait_for_timeout(1000)
                        if upload_census_file(page):
                            ...
                            if fill_benefit_details(page):
                             ...
                             # --- SAVE QUOTE AND DOWNLOAD PDF ---
                                ...
                                if save_quote_and_download_pdf(page,context):
                                    print("Quote saved and PDF downloaded successfully!")
